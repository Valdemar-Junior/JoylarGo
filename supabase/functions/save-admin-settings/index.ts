...
type Payload = {
  auth_token?: string;
  app_settings?: { key: string; value: unknown; updated_at?: string }[];
};
...
Deno.serve(async (req) => {
  ...
  const rows = Array.isArray(payload.app_settings) ? payload.app_settings : [];
  ...
  if (requesterProfileError || requesterProfile?.role !== "admin") {
    return json(403, { error: "Only admins can save settings." });
  }
  ...
  const normalizedRows = rows
    .map((row) => ({
      ...
      updated_by: requesterUser.user!.id,
    }))
    .filter((row) => row.key);

  const { error } = await admin.from("app_settings").upsert(normalizedRows, { onConflict: "key" });
  if (error) {
    return json(500, { error: error.message || "Failed to save app settings." });
  }
