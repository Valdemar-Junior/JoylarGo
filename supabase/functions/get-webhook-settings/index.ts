...
type GetWebhookSettingsPayload = {
  auth_token?: string;
};
...
Deno.serve(async (req) => {
  ...
  let payload: GetWebhookSettingsPayload = {};
  try {
  ...
  if (requesterProfileError || requesterProfile?.role !== "admin") {
    return json(403, { error: "Only admins can read webhook settings." });
  }

  const { data, error } = await admin
    .from("webhook_settings")
    .select("key,url,active,updated_at")
    .order("key");

  if (error) {
    return json(500, { error: error.message || "Failed to load webhook settings." });
  }
