...
export default function Settings() {
  ...
  const load = async () => {
    try {
      ...
      const webhooksPromise = fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/get-webhook-settings`, {
        method: 'POST',
        headers: {
        ...
        if (!response.ok) throw new Error(payload?.error || 'Erro ao carregar webhooks');
        return payload?.rows || [];
      });

      const webhookRows = await webhooksPromise;
      const [confFlag, updateFlag, photoFlag, deliveryPhotoFlag, deliveryProofFlag, ruralKeys, generalDeadlines] = await Promise.allSettled([
      ...
      const webhookMap = new Map(
        ((webhookRows as any[]) || [])
          ...
          .map((row: any) => [row.key, row.url])
      );

      setEnviaPedidos((webhookMap.get('envia_pedidos') as string) || '');
      setGeraNf((webhookMap.get('gera_nf') as string) || '');
      setGeraNfDevolucao((webhookMap.get('gera_nf_devolucao') as string) || '');
      setEnviaMensagem((webhookMap.get('envia_mensagem') as string) || '');
      setEnviaGrupo((webhookMap.get('envia_grupo') as string) || '');
      setConsultaLancamento((webhookMap.get('consulta_lancamento') as string) || '');

      const enabled = confFlag.status === 'fulfilled' ? (confFlag.value.data as any)?.value?.enabled : undefined;
      setRequireConference(enabled === false ? false : true);

      const updatesEnabled = updateFlag.status === 'fulfilled' ? (updateFlag.value.data as any)?.value?.enabled : undefined;
      setAllowOrderUpdates(updatesEnabled === true); // Default to false if missing

      const photosEnabled = photoFlag.status === 'fulfilled' ? (photoFlag.value.data as any)?.value?.enabled : undefined;
      setRequireAssemblyPhotos(photosEnabled === true); // NOVO: Default to false

      const deliveryPhotosEnabled = deliveryPhotoFlag.status === 'fulfilled' ? (deliveryPhotoFlag.value.data as any)?.value?.enabled : undefined;
      setRequireDeliveryPhotos(deliveryPhotosEnabled === true); // Default false

      const deliveryProofSettings = (deliveryProofFlag.status === 'fulfilled' ? (deliveryProofFlag.value.data as any)?.value : {}) || {};
      ...
      const keywords = ruralKeys.status === 'fulfilled' ? (ruralKeys.value.data as any)?.value?.keywords : undefined;
      setRuralKeywords(Array.isArray(keywords) ? keywords : []);

      const general = generalDeadlines.status === 'fulfilled' ? (generalDeadlines.value.data as any)?.value : undefined;
  ...
  const save = async () => {
    try {
      ...
      if (activeTab === 'integrations') {
        const functionUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/save-webhook-settings`;
        const response = await fetch(functionUrl, {
          ...
          body: JSON.stringify({ rows, auth_token: accessToken }),
        });
        const payload = await response.json().catch(() => null);
        if (!response.ok) throw new Error(payload?.error || 'Erro ao salvar webhooks');
