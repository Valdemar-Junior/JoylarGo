...
export default function UsersTeams() {
  ...
  const loadAll = async () => {
    try {
      setLoading(true);
      const { data: usersData, error: usersError } = await supabase.rpc('admin_list_users');
      ...
      if (teamsData) {
        setTeams(teamsData.map((team: any) => ({
          ...team,
          driver: { name: team.driver_name },
          helper: { name: team.helper_name },
        })));
      }

      const { data: vehiclesData, error: vehiclesError } = await supabase.rpc('admin_list_vehicles');
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const resp = await fetch(functionUrl, {
                          ...
                          body: JSON.stringify({ userId: user.id, newPassword: temp, auth_token: accessToken })
                        });
                        if (!resp.ok) {
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const resp = await fetch(functionUrl, {
                          ...
                          body: JSON.stringify({ userId: user.id, auth_token: accessToken })
                        });
                        if (!resp.ok) {
                          let msg = 'Falha ao remover usuario';
                          try { const j = await resp.json(); if (j?.error) msg = j.error; } catch { }
                          throw new Error(msg);
                        }
