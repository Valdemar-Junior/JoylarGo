...
export default function FirstLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const functionUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/complete-first-login`;
      ...
      if (!response.ok) {
        throw new Error(payload?.error || 'Erro ao atualizar seu perfil');
      }

      toast.success('Senha alterada com sucesso');
      let target = '/';
      const refreshed = payload?.profile || null;
      if (refreshed) {
